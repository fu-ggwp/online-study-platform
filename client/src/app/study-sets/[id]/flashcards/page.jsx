...
import Flashcard from "@/app/learner/study-sets/[id]/flashcards/_components/Flashcard";
import FlashcardProgress from "@/app/learner/study-sets/[id]/flashcards/_components/FlashcardProgress";
...
export default function PublicStudySetFlashcardsPage() {
  ...
  // --- Initialize data on mount / auth change ---
  useEffect(() => {
    ...
    initSession();
  }, [studySetId, authLoading, isAuthenticated]);
  ...
  }, [currentIndex, questions, studySetId, isAuthenticated]);
  ...
  const saveProgress = async (qId, status) => {
    if (!sessionId) return; // Skip saving for guest users
    try {
  ...
  // Congrats screen when learner has finished all cards
  if (currentIndex === questions.length) {
    return (
        ...
        <main className="h-[calc(100vh-80px)] max-h-[calc(100vh-80px)] overflow-hidden bg-background text-foreground flex flex-col justify-between py-4 px-4 md:px-8">
          ...
          <section className="flex-1 max-w-md w-full mx-auto flex flex-col justify-center items-center text-center space-y-6 px-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full">
              ...
              <Button onClick={() => setShowLoginModal(true)} variant="outline" className="flex-1 rounded-xl font-semibold py-2.5">
                Practice Quiz
              </Button>
            </div>
          </section>
