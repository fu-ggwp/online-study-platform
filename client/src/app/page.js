...
export default function Home() {
  ...
  const roleHome = isAuthenticated ? getRoleHome(role) : null;

  useEffect(() => {
    if (loading || !roleHome) return;

    router.replace(roleHome);
  }, [loading, roleHome, router]);

  if (loading || roleHome) {
    return <LoadingHomeRedirect />;
  }
